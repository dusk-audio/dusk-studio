...
if ($CMakeCache -notmatch '(?m)^DUSKSTUDIO_ENABLE_NATIVE_UI:BOOL=ON\r?$') {
    throw "Windows MSI packaging requires DUSKSTUDIO_ENABLE_NATIVE_UI=ON (a cached OFF is sticky). Reconfigure explicitly with a valid DAF_PATH checkout containing widgets/, then rebuild."
}
