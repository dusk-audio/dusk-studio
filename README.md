...
The C++ suite declares 737 Catch2 test cases across 148 test source files. Linux (amd64 + arm64) + macOS + Windows builds run on every push; Windows tests run on every push + PR; Linux ThreadSanitizer runs on every PR + push.
...
tests/         # 737 Catch2 test cases declared in C++ (session, recording, MIDI, IPC, DSP)
