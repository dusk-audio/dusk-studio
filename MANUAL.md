...
- **The notepad** needs a display that provides OpenGL 3. Ordinary desktop graphics on all three platforms do, but a Windows remote-desktop session or a virtual machine with a basic display adapter can fall back to a renderer that does not. Dusk Studio then reports that the notepad cannot be embedded, and the rest of the application is unaffected.
