...
- **The notepad** needs a display that provides OpenGL 3. Ordinary desktop graphics on all three platforms do, but a Windows remote-desktop session or a virtual machine with a basic display adapter can fall back to a renderer that does not. One Windows renderer is refused even though it reports OpenGL 4.6: the Mesa GL-on-D3D12 driver that Microsoft ships as the OpenGL Compatibility Pack, which a machine without a vendor OpenGL driver gets from the Store. It ends the whole application while drawing the notepad's first frame, so Dusk Studio declines to open the notepad there. Installing the graphics vendor's own driver restores it. In every refused case Dusk Studio reports that the notepad cannot be embedded, and the rest of the application is unaffected.
- **If the notepad ever takes Dusk Studio down with it**, it will not do so twice. A graphics driver that ends the application while the notepad is drawing its first frame leaves a marker behind, and the next launch refuses the notepad and names the renderer that did it. The rest of Dusk Studio is unaffected. To try again after changing graphics drivers, delete the `notepad-first-frame` file in Dusk Studio's configuration folder. To keep the notepad switched off permanently, set `notepad_enabled=0` in `app-config.properties` in that same folder.
